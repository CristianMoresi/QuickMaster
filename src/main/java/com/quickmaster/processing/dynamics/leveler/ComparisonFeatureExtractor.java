...
public final class ComparisonFeatureExtractor
{
                                         ...
                                         byte[] flags, int stride)
    {
        ...
        double[] hannWindow = new double[support];
        ...
        double[] timbreEdges = null;
        if (stride == 52)
        {
            timbreEdges = new double[9];
            double upper = Math.min(16000.0d, .95d * sampleRate / 2.0d);
            if (upper > 60.0d)
            {
                double ratio = upper / 60.0d;
                // The same deterministic boundaries apply to every FFT row.
                for (int band = 0; band < 8; band++)
                    timbreEdges[band] = 60.0d * StrictMath.pow(ratio, band / 8.0d);
                timbreEdges[8] = upper;
            }
        }
        ...
        for (int row = 0; row < flags.length; row++)
        {
            ...
            for (int channel = 0; channel < channels; channel++)
            {
                ...
                for (int n = 0; n < support; n++)
                {
                    ...
                    time[n] = (float) (sample * hannWindow[n]);
                }
                if (cancelled(token)) return false;
            ...
            if (stride == 52)
            {
                if (!timbre(power, sampleRate, size, scratch, components, timbreEdges, token))
                {
                    if (cancelled(token)) return false;
                    continue;
                }
                                 ...
                                 CancellationToken token)
    {
        ...
        for (int bin = 1; bin < power.length - 1; bin++)
        {
            ...
            for (int component = 0; component < 36; component++)
            {
                ...
                // The cosine factor is in [0,1]. If the existing maximum is
                // already at least the full salience, no cosine can change it.
                if (distance <= 1.0d && components[component] < salience)
                    components[component] = Math.max(components[component],
                            salience * (1.0d + Math.cos(StrictMath.PI * distance)) / 2.0d);
            }
        }
        double sum = 0.0d;
    ...
    private static boolean timbre(double[] power, int sampleRate, int size, double[] scratch,
                                  double[] components, double[] bandEdges, CancellationToken token)
    {
        double upper = Math.min(16000.0d, .95d * sampleRate / 2.0d);
        ...
        {
            if (cancelled(token)) return false;
            double low = bandEdges[band];
            double high = bandEdges[band + 1];
            int count = 0;
            double energy = 0.0d;
            for (int bin = 1; bin < power.length; bin++)
            {
